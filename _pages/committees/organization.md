---
title: Organizing Committee
layout: single
excerpt: "AACL-IJCNLP 2026 organizing committee."
permalink: /organization/
sidebar:
  nav: "committees"
---

<h2>General Chair</h2>
{% include committee-member.html
   name="Yue Zhang"
   institution="Westlake University"
   email="zhangyue@westlake.edu.cn"
   role="General Chair"
   picture="/assets/images/committee/Yue-Zhang.jpg"
%}

<h2>Program Chairs</h2>
{% include committee-member.html
   name="Ruifeng Xu"
   institution="Harbin Institute of Technology, Shenzhen"
   email="xuruifeng@hit.edu.cn"
   role="Program Chairs"
   picture="/assets/images/committee/Ruifeng-Xu.jpeg"
%}
{% include committee-member.html
   name="Ayu Purwarianti"
   institution="Institut Teknologi Bandung"
   email="ayu@staff.stei.itb.ac.id"
   role="Program Chairs"
   picture="/assets/images/committee/Ayu-Purwarianti-2026.jpeg"
%}
{% include committee-member.html
   name="Tong Rong"
   institution="Singapore Institute of Technology"
   email="tong.rong@singaporetech.edu.sg"
   role="Program Chairs"
   picture="/assets/images/committee/Tong-Rong.jpg"
%}
{% include committee-member.html
   name="Jun Suzuki"
   institution="Tohoku University"
   email="jun.suzuki@tohoku.ac.jp"
   role="Program Chairs"
   picture="/assets/images/committee/Jun-Suzuki-2026.jpeg"
%}

<h2>Local Organizing Chair</h2>
{% include committee-member.html
   name="Derek F. Wong"
   institution="University of Macau"
   email="derekfw@um.edu.mo"
   role="Local Organizing Chair"
   picture="/assets/images/committee/Derek-F-Wong.png"
%}
{% include committee-member.html
   name="Haofen Wang"
   institution="Tongji University"
   email="haofen.wang@tongji.edu.cn"
   role="Local Organizing Chair"
   picture="/assets/images/committee/Haofen-Wang.jpg"
%}

<h2>Local Organizing Committees</h2>
{% include committee-member.html
   name="Runzhe Zhan"
   institution="University of Macau"
   email="nlp2ct.runzhe@gmail.com"
   role="Local Organizing Committees"
   picture="/assets/images/committee/Runzhe-Zhan.jpg"
%}
{% include committee-member.html
   name="Zhiwen Xie"
   institution="Central China Normal University"
   email="zwxie@ccnu.edu.cn"
   role="Local Organizing Committees"
   picture="/assets/images/committee/Zhiwen-Xie.jpg"
%}

<h2>Workshop Chairs</h2>
{% include committee-member.html
   name="Xuefeng Bai"
   institution="Harbin Institute of Technology, Shenzhen"
   email="baixuefeng@hit.edu.cn"
   role="Workshop Chairs"
   picture="/assets/images/committee/Xuefeng-Bai.png"
%}
{% include committee-member.html
   name="Ratish Puduppully"
   institution="IT University of Copenhagen"
   email="ratishpuduppully@gmail.com"
   role="Workshop Chairs"
   picture="/assets/images/committee/Ratish-Puduppully.jpg"
%}

<h2>Tutorial Chairs</h2>
{% include committee-member.html
   name="Zhongqing Wang"
   institution="Soochow University"
   email="wangzq@suda.edu.cn"
   role="Tutorial Chairs"
   picture="/assets/images/committee/Zhongqing-Wang.jpg"
%}
{% include committee-member.html
   name="Naoaki Okazaki"
   institution="Institute of Science Tokyo"
   email="okazaki@comp.isct.ac.jp"
   role="Tutorial Chairs"
   picture="/assets/images/committee/Naoaki-Okazaki.jpg"
%}

<h2>Demonstration Chairs</h2>
{% include committee-member.html
   name="Ningyu Zhang"
   institution="Zhejiang University"
   email="zhangningyu@zju.edu.cn"
   role="Demonstration Chairs"
   picture="/assets/images/committee/Ningyu-Zhang.jpeg"
%}
{% include committee-member.html
   name="Derry Wijaya"
   institution="Monash University, Indonesia"
   email="derry.wijaya@monash.edu"
   role="Demonstration Chairs"
   picture="/assets/images/committee/Derry-Wijaya.jpg"
%}

<h2>Student Research Workshop Chair: Faculty Advisors</h2>
{% include committee-member.html
   name="Yixin Cao"
   institution="Fudan University"
   email="yxcao@fudan.edu.cn"
   role="Student Research Workshop Chair"
   picture="/assets/images/committee/Yixin-Cao.png"
%}

<h2>Student Research Workshop Chairs</h2>
{% include committee-member.html
   name="Shibo Hong"
   institution="Fudan University"
   email="hongshibo02@gmail.com"
   role="Student Research Workshop Chair"
   picture="/assets/images/committee/Shibo-Hong.jpg"
%}
{% include committee-member.html
   name="Yuhang Zhou"
   institution="Shanghai Innovation Institute"
   email="yuhangzhou22@m.fudan.edu.cn"
   role="Student Research Workshop Chair"
   picture="/assets/images/committee/Yuhang-Zhou.jpg"
%}

<h2>Best Paper Award Chair</h2>
{% include committee-member.html
   name="Rui Yan"
   institution="Wuhan University"
   email="rui.yan@whu.edu.cn"
   role="Best Paper Award Chair"
   picture="/assets/images/committee/Rui-Yan.jpeg"
%}

<h2>Publication Chairs</h2>
{% include committee-member.html
   name="Junxian He"
   institution="Hong Kong University of Science and Technology"
   email="junxianh@cse.ust.hk"
   role="Publication Chairs"
   picture="/assets/images/committee/Junxian-He.jpg"
%}
{% include committee-member.html
   name="Trang Vu"
   institution="Monash University"
   email="trang.vu1@monash.edu"
   role="Publication Chairs"
   picture="/assets/images/committee/Trang-Vu.jpg"
%}

<h2>Handbook Chair</h2>
{% include committee-member.html
   name="Leyang Cui"
   institution="Tencent AI Lab"
   email="nealcly.nlp@gmail.com"
   role="Handbook Chair"
   picture="/assets/images/committee/Leyang-Cui.jpeg"
%}

<h2>Publicity Chairs</h2>
{% include committee-member.html
   name="Aditya Joshi"
   institution="The University of New South Wales"
   email="aditya.joshi@unsw.edu.au"
   role="Publicity Chairs"
   picture="/assets/images/committee/Aditya-Joshi-2026.jpeg"
%}
{% include committee-member.html
   name="KyungTae Lim"
   institution="Korea Advanced Institute of Science & Technology"
   email="ktlim@kaist.ac.kr"
   role="Publicity Chairs"
   picture="/assets/images/committee/KyungTae-Lim.jpeg"
%}

<h2>Web and App Chairs</h2>
{% include committee-member.html
   name="Xinchi Chen"
   institution="Fudan University"
   email="xc_chen@fudan.edu.cn"
   role="Web and App Chairs"
   picture="/assets/images/committee/Xinchi-Chen.jpeg"
%}
{% include committee-member.html
   name="Piji Li"
   institution="Nanjing University of Aeronautics and Astronautics"
   email="pjli@nuaa.edu.cn"
   role="Web and App Chairs"
   picture="/assets/images/committee/Piji-Li.jpeg"
%}
{% include committee-member.html
   name="Shudong Liu"
   institution="University of Macau"
   email="nlp2ct.shudong@gmail.com"
   role="Web and App Chairs"
   picture="/assets/images/committee/Shudong-Liu.jpeg"
%}

<h2>Finance Chair</h2>
{% include committee-member.html
   name="Hou Pong Chan"
   institution="Alibaba DAMO Academy"
   email="kenchanhp@gmail.com"
   role="Finance Chair"
   picture="/assets/images/committee/Hou-Pong-Chan.jpeg"
%}

<h2>Registration Chair</h2>
{% include committee-member.html
   name="Wenxuan Zhang"
   institution="Singapore University of Technology and Design"
   email="wxzhang@sutd.edu.sg"
   role="Registration Chair"
   picture="/assets/images/committee/Wenxuan-Zhang.jpeg"
%}

<h2>Ethics Chairs</h2>
{% include committee-member.html
   name="Yangqiu Song"
   institution="Hong Kong University of Science and Technology"
   email="yqsong@cse.ust.hk"
   role="Ethics Chairs"
   picture="/assets/images/committee/Yangqiu-Song.jpeg"
%}
{% include committee-member.html
   name="Ekaterina Vylomova"
   institution="The University of Melbourne"
   email="ekaterina.vylomova@unimelb.edu.au"
   role="Ethics Chairs"
   picture="/assets/images/committee/Ekaterina-Vylomova.jpeg"
%}

<h2>Diversity & Inclusion Chair</h2>
{% include committee-member.html
   name="Mamoru Komachi"
   institution="Hitotsubashi University"
   email="mamoru.komachi@r.hit-u.ac.jp"
   role="Diversity & Inclusion Chair"
   picture="/assets/images/committee/Mamoru-Komachi.jpeg"
%}

<h2>Student Volunteer Chair</h2>
{% include committee-member.html
   name="Juntao Li"
   institution="Soochow University"
   email="ljt@suda.edu.cn"
   role="Student Volunteer Chair"
   picture="/assets/images/committee/Juntao-Li.jpeg"
%}

<h2>Sponsorship Chairs</h2>
{% include committee-member.html
   name="Yan Song"
   institution="University of Science and Technology of China"
   email="songyan@ustc.edu.cn"
   role="Sponsorship Chairs"
   picture="/assets/images/committee/Yan-Song.jpg"
%}
{% include committee-member.html
   name="Yuanhe Tian"
   institution="Zhongguancun Institute of Artificial Intelligence"
   email="tianyuanhe@zgci.ac.cn"
   role="Sponsorship Chairs"
   picture="/assets/images/committee/Yuanhe-Tian.jpeg"
%}
